...
def handle_user_list(user_service: UserService):
    ...
    # TODO: 나중에 삭제할 코드 - Slack API로 전체 사용자 목록 가져오기
    try:
        if SLACK_BOT_TOKEN:
            logger.info("Slack API를 통해 전체 사용자 목록을 가져오는 중...")
            ...
            if response.status_code == 200:
                ...
                if data.get("ok"):
                    ...
                    active_users = [user for user in users if not user.get("deleted") and not user.get("is_bot")]
                    ...
                    for user in active_users:
                        ...
                        # 표시할 이름 결정 (real_name > display_name > name 순서)
                        display_name_final = real_name or display_name or name
                        
                        if user_id and display_name_final:
                            slack_user_mapping[user_id] = display_name_final
                    
                    logger.info(f"Slack에서 {len(slack_user_mapping)}명의 사용자를 가져왔습니다.")
                    
                    # Slack 사용자 목록을 문자열로 변환
                    slack_users_text = "🔍 *Slack 워크스페이스 전체 사용자 목록*\n"
                    ...
                    if result["success"]:
                        if not result["users"]:
                            return {
                                "response_type": "ephemeral",
                                "text": f"{slack_users_text}\n\n👥 *DB 사용자 목록*\n아직 등록된 사용자가 없습니다.\n`/사용자등록`으로 사용자를 등록해보세요!"
                            }
                        
                        db_response_text = "👥 *DB 사용자 목록*\n"
                        for user in result["users"]:
                            db_response_text += f"• *{user['name']}* (<@{user['user_id']}>)\n"
                            db_response_text += f"  포지션: {user['position'] or '미입력'}\n"
                            db_response_text += f"  학교/전공: {user['school_major'] or '미입력'}\n"
                            db_response_text += f"  4대보험: {user['insurance'] or '미입력'}\n\n"
                        ...
                        return {
                            "response_type": "ephemeral",
                            "text": f"{slack_users_text}\n\n❌ DB 사용자 목록 조회 실패: {result['message']}"
                        }
                else:
                    logger.warning(f"Slack API 오류: {data.get('error')}")
            else:
                logger.warning(f"Slack API HTTP 오류: {response.status_code}")
        else:
            logger.warning("SLACK_BOT_TOKEN이 설정되지 않았습니다.")
    except Exception as e:
        logger.error(f"Slack API 호출 중 오류: {e}")
